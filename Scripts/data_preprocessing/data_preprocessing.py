...
def main():
    ...
    # Once this is done, create some plots
    ...
    #
    # sns.barplot(data=genre_counts, x='Genre', y='Count', palette='pastel', ax=ax0)
    # ax0.set_title("Distribution of Games by Genre")
    ...
    # ax0.axhline(y=mean_count, color='red', linestyle='dashed',
    #             linewidth=2, label=f"Mean: {mean_count:.2f}")
    # ax0.legend()
    #
    # plt.show()

    # 2nd chart for Gamesdf
    # ig, ax0 = plt.subplots(figsize=(30, 10))
    #
    # # Convert year to integer to avoid an extra .0
    # year_counts = games_df['Year'].astype(int).value_counts().sort_index().reset_index()
    # year_counts.columns = ['Year', 'Number of Games']
    #
    # # Count the number of games released each year
    # year_counts = games_df['Year'].value_counts().sort_index().reset_index()
    # year_counts.columns = ['Year', 'Number of Games']
    #
    # # Create a bar chart showing games per year
    # sns.barplot(data=year_counts, x='Year', y='Number of Games', hue='Year', palette='pastel', ax=ax0)
    # ax0.set_title("Number of Games per Year", fontsize=16)
    # ax0.set_xlabel("Year", fontsize=14)
    # ax0.set_ylabel("Number of Games", fontsize=14)
    #
    # # Calculate statistics for annotation
    # mean_games = year_counts['Number of Games'].mean()
    # median_games = year_counts['Number of Games'].median()
    #
    ...
    # sns.histplot(data=audio_df, x='Year', kde=True, ax=ax)
    #
    # # Add title and labels
    # ax.set_title('Distribution of Audio Tracks by Year', fontsize=14)
    # ax.set_xlabel('Year', fontsize=10)
    # ax.set_ylabel('Count', fontsize=10)
    #
    # plt.show()

    # Song df visualization
    # Correlation heatmap
    plt.figure(figsize=(10, 8))
    numeric_features = song_df[['popularity', 'energy', 'tempo', 'valence']]
